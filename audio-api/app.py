...
def practice_tracks():
    ...
    print(f"Received {len(files)} files:", files)

    if len(files) == 0:
        return make_response(jsonify({"error": "No files uploaded"}, 400))

    # create temp dir
    dir_path = os.path.join("uploads", str(uuid.uuid4()))
    os.makedirs(dir_path, exist_ok=True)
    print(f"Created temp dir at {os.path.abspath(dir_path)}")
    ...
    try:
        ...
        print(f"Deleted temp dir at {os.path.abspath(dir_path)}")
